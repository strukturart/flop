...
function setupConnectionEvents(conn) {
  ...
  conn.on("data", function (data) {
    ...
    try {
      ...
      addressbook.forEach((e) => {
        if (e.id == data.from) {
          if (!e.client_id || e.client_id != clientID) {
      ...
      if (!status.webpush_do_not_annoy.includes(status.current_clientId)) {
        ...
        setTimeout(() => {
          ...
          if (index !== -1) {
            ...
            console.log(`Client ${status.current_clientId} removed`);
          }
        }, 5 * 60 * 1000);
      }
    } else {
                    ...
                    "button",
                    {
                      ...
                      onclick: () => {
                        if (e.live == true) {
                          ...
                          if (pid != settings.unique_id) {
                            status.current_clientId = pid;

                            //sendPushMessage(status.current_clientId, "Flop");
                          } else {
                            side_toaster("can't send pushMessage", 3000);
                          }
                        }
                      },
                    },
