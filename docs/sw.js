    ...
    var serviceHandler = ()=>{
        if (evt.name === "activity") {
            ...
            if (handler.source.name == "flop") {
                (0, (/*@__PURE__*/$parcel$interopDefault($9hfyQ))).setItem("connect_to_id", handler.source.data).then((e)=>{});
...
self.addEventListener("notificationclick", (event)=>{
    ...
    event.waitUntil(clients.matchAll({
        ...
        if (clients.openWindow) return clients.openWindow(new URL("/", self.location.origin)).then((w)=>w.focus());
        if (clients.openApp) return clients.openApp();
    }).catch((err)=>{
        console.log(err);
    }));
});
...
if ($857385147de8908e$var$userAgent && !$857385147de8908e$var$userAgent.includes("KAIOS")) {
    var $857385147de8908e$var$CACHE_NAME = "pwa-cache-v0.2105";
    ...
    self.addEventListener("install", (event)=>{
        event.waitUntil(caches.open($857385147de8908e$var$CACHE_NAME).then((cache)=>{
            ...
            return fetch($857385147de8908e$var$FILE_LIST_URL).then((response)=>{
                if (!response.ok) throw new Error(`HTTP error! Status: ${response.status}`);
                return response.json(); // Parse the JSON response
            }).then((urlsToCache)=>{
                // Ensure urlsToCache is an array
                if (Array.isArray(urlsToCache)) return Promise.all(urlsToCache.map((url)=>cache.add(url).catch((error)=>{
                        console.error(`Failed to cache ${url}:`, error);
                    })));
                else console.error("Fetched data is not an array:", urlsToCache);
            });
        }).then(()=>{
    ...
    self.addEventListener("activate", (event)=>{
        var cacheWhitelist = [
            $857385147de8908e$var$CACHE_NAME
        ];
...
parcelRegister("9hfyQ", function(module, exports) {
/*!
...
*/ (function(f) {
    ...
    return (function e(t, n, r) {
        ...
        4: [
            function(_dereq_, module1, exports) {
                'use strict';
                var _typeof = typeof Symbol === "function" && typeof Symbol.iterator === "symbol" ? function _typeof(obj) {
                    return typeof obj;
                } : function(obj) {
